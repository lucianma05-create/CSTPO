"""SFT 样本 tokenize + loss 掩码（Qwen3 chat template 精确实现）。

口径（SFT_SPEC 2.2 / sft_config.MASK_LABEL_LINE）：
- input_ids：messages 按 Qwen3 apply_chat_template 渲染；
- labels：最后一条 assistant 消息参与 loss——MASK_LABEL_LINE=True 时仅
  "话语部分"（原话语口径）；False 时"标签行+话语"全量学习（2026-09-18
  裁定 B：模型需真正学会标签语义）。历史 assistant、system/user、模板
  结构标记恒为 IGNORE。

实现：把最后一条 assistant 拆成 [标签行][话语] 两段单独编码，
使标签行 token 边界可直接定位（不依赖字符串搜索）。

自检：python -m cstpo.sft.sft_tokenize --self-check
"""
from __future__ import annotations

import json
import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parents[2]
sys.path.insert(0, str(ROOT))

from cstpo.sft.sft_config import LABEL_SEP, MASK_LABEL_LINE, MAX_SEQ_LEN, MODEL_ID

IGNORE = -100


def tokenize_sample(tk, sample: dict) -> dict:
    """→ {"input_ids": list[int], "labels": list[int]}（未截断）。"""
    messages = sample["messages"]
    label = sample["label"]
    last = messages[-1]
    assert last["role"] == "assistant"
    content = last["content"]
    assert content.startswith(label + LABEL_SEP), \
        f"标签前缀不符: {content[:40]!r} vs {label!r}"
    utterance = content[len(label + LABEL_SEP):]

    # 1) 前缀部分：历史消息 + generation prompt，与推理侧（ab_validate/eval/
    # RL agent loop 的 add_generation_prompt=True）逐 token 一致。旧实现用
    # "空 assistant 内容 + add_generation_prompt=False" 渲染——实际模板会把
    # 空回合渲染成 "assistant\n<think>\n\n</think>\n\n<|im_end|>\n"，导致
    # 标签/话语被教在 <|im_end|> 之后的位置，而该位置在推理中不存在
    # （v1/v2 标签 0 输出的根因，2026-09-18 定位）。
    prefix_ids = list(tk.apply_chat_template(
        messages[:-1], tokenize=True, add_generation_prompt=True,
        enable_thinking=False)["input_ids"])

    # 2) 目标回合内容：标签行 + 话语（学习范围由 MASK_LABEL_LINE 决定）
    label_ids = tk.encode(label + LABEL_SEP, add_special_tokens=False)
    utter_ids = tk.encode(utterance, add_special_tokens=False)

    # 3) 结尾结构标记（<|im_end|>\n，不参与 loss）
    tail = tk.encode("<|im_end|>\n", add_special_tokens=False)

    # 全量口径只学真实策略标签；<unlabeled> 是数据缺失占位（补标后仅零散
    # 残留），维持 mask——模型不应学会输出它
    label_target = label_ids if (not MASK_LABEL_LINE and label != "<unlabeled>") \
        else [IGNORE] * len(label_ids)
    input_ids = prefix_ids + label_ids + utter_ids + tail
    labels = [IGNORE] * len(prefix_ids) + label_target \
        + utter_ids + [IGNORE] * len(tail)
    # system 段长度（截断时保留 system + 尾部——对话样本关键信息在后）
    sys_ids = list(tk.apply_chat_template(
        [{"role": "system", "content": messages[0]["content"]}],
        tokenize=True, add_generation_prompt=False,
        enable_thinking=False)["input_ids"])
    return {"input_ids": input_ids, "labels": labels, "system_len": len(sys_ids)}


def left_truncate(out: dict, max_len: int) -> dict:
    """左截断：保留 system 段 + 序列尾部（对话样本关键信息在后）。"""
    sys_len = out.get("system_len", 0)
    ids, labs = out["input_ids"], out["labels"]
    if len(ids) <= max_len:
        return {"input_ids": ids, "labels": labs}
    keep_tail = max_len - sys_len
    return {"input_ids": ids[:sys_len] + ids[-keep_tail:],
            "labels": labs[:sys_len] + labs[-keep_tail:]}


def tokenize_and_pad(tk, sample: dict) -> dict:
    out = tokenize_sample(tk, sample)
    out = left_truncate(out, MAX_SEQ_LEN)
    ids, labs = out["input_ids"], out["labels"]
    pad = MAX_SEQ_LEN - len(ids)
    ids += [tk.pad_token_id] * pad
    labs += [IGNORE] * pad
    return {"input_ids": ids, "labels": labs, "attention_mask": [1] * len(ids)}


def load_tokenizer():
    from transformers import AutoTokenizer
    tk = AutoTokenizer.from_pretrained(MODEL_ID)
    if tk.pad_token_id is None:
        tk.pad_token_id = tk.eos_token_id
    return tk


def self_check():
    tk = load_tokenizer()
    sample = {"messages": [
        {"role": "system", "content": "You are a helpful and caring friend."},
        {"role": "user", "content": "i am feeling sad."},
        {"role": "assistant", "content": "Question\nWhat happened?"},
    ], "label": "Question"}
    out = tokenize_sample(tk, sample)
    ids, labs = out["input_ids"], out["labels"]
    # 断言：有可学习 token；labels 与 input 对齐；话语 tokens 完整保留
    assert len(ids) == len(labs)
    learn = [i for i, l in zip(ids, labs) if l != IGNORE]
    assert learn, "无学习 token"
    text = tk.decode(learn)
    assert "What happened?" in text, f"话语缺失: {text!r}"
    n_learn = sum(1 for l in labs if l != IGNORE)
    n_label = len(tk.encode("Question\n", add_special_tokens=False))
    n_utter = len(tk.encode("What happened?", add_special_tokens=False))
    if MASK_LABEL_LINE:
        assert "Question" not in text, f"标签行未被 mask: {text!r}"
        assert n_learn == n_utter
    else:
        assert "Question" in text, f"标签行未参与学习: {text!r}"
        assert n_learn == n_utter + n_label
    mode = "mask" if MASK_LABEL_LINE else "learn"
    print(f"self-check PASS: 学习 token {n_learn} 个（标签行 {n_label} 个 {mode}），"
          f"总长 {len(ids)}")
    # 截断自检
    p = tokenize_and_pad(tk, sample)
    assert len(p["input_ids"]) == MAX_SEQ_LEN == len(p["labels"])
    print(f"pad 自检 PASS: {MAX_SEQ_LEN}")


if __name__ == "__main__":
    self_check()
