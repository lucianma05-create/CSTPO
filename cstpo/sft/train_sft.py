...
- 掩码：sft_tokenize（MASK_LABEL_LINE 控制标签行口径；2026-09-18 起全量学习）；
...
from cstpo.sft.sft_config import (BATCH_SIZE, BF16, DATA_DIR, EPOCHS,
                              ...
                              WEIGHT_DECAY)
from cstpo.sft.sft_tokenize import IGNORE, left_truncate, tokenize_sample
...
def load_mixed(task: str, real_ratio: float = MIX_REAL_RATIO) -> list[dict]:
    ...
    if van:
        ...
        if keep_real < len(real):
            ...
            # 按历史长度分桶，桶内按桶规模比例采样（修 2026-09-18 发现的
            # 桶限欠采样：旧实现每桶 cap=keep_real//n_buckets，样本集中
            # 时实际保留数远低于 keep_real）
            buckets = {}
            for s_ in real:
                n = len(s_["messages"])
            ...
            for k in sorted(buckets):
                b = buckets[k]
                keep_b = max(1, round(keep_real * len(b) / total))
                real += rng.sample(b, min(len(b), keep_b))
