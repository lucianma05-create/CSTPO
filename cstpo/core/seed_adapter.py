...
def build_esconv_seed(record: dict, seed_id: str, seed_rng: random.Random) -> dict:
    ...
    if cut is not None:
        ...
        # v5.3：统一两轮前缀——无实质披露的对话也取前两条 user 消息作
        # rollout 前缀（与所有种子同逻辑，无空前缀特例）；完整对话前
        # 12 轮作为提取上下文（潜在主观状态推断依据）
        all_msgs = [{"role": "seeker" if m["from"] == "human" else "supporter",
                     "text_en": m["value"], "original_index": i}
                    for i, m in enumerate(record["conversations"][:12])]
        extraction_context = all_msgs
        n_user = 0
        for m in all_msgs:
            prefix.append(m)
            if m["role"] == "seeker":
                ...
                if n_user >= 2:
                    break
    # v5.4：全量对话（含截止点之后）作为标注/审阅参考——提取模型看到全部、
    # 前端渲染全部；rollout 可见历史仍由 prefix_turns 截取
    full_conversation = [
        {"role": "seeker" if m["from"] == "human" else "supporter",
         "text_en": m["value"], "original_index": i}
        for i, m in enumerate(record["conversations"])]
    ...
    seed = {
        ...
        "context": {"prefix": prefix,
                    ...
                    "cutoff": cut if cut else {"rule": "前 4 回合无实质披露 → 统一前两条 user 消息前缀"},
...
def build_p4g_seed(record: dict, seed_id: str, seed_rng: random.Random) -> dict:
    ...
    if cut is not None:
        ...
        # v5.3：统一两轮前缀——无实质 persuadee 消息的对话也取前两条
        # user 消息作 rollout 前缀；完整对话前 12 轮作提取上下文
        all_msgs = [{"role": "persuadee" if m["from"] == "human" else "persuader",
                     "text_en": m["value"], "original_index": i}
                    for i, m in enumerate(record["conversations"][:12])]
        extraction_context = all_msgs
        n_user = 0
        for m in all_msgs:
            ...
            if m["role"] == "persuadee":
                ...
                if n_user >= 2:
                    break
    full_conversation = [
        {"role": "persuadee" if m["from"] == "human" else "persuader",
         "text_en": m["value"], "original_index": i}
        for i, m in enumerate(record["conversations"])]
    ...
    seed = {
        ...
        "context": {"prefix": prefix,
                    "full_conversation": full_conversation,
                    "extraction_context": extraction_context,
                    "cutoff": cut if cut else {"rule": "无实质 persuadee 消息 → 统一前两条 user 消息前缀"},
                    "next_speaker": "persuader",
                    "message_indices": [p["original_index"] for p in prefix]},
        "runtime": {"init_mode": "prefix_reset", "route_mode": "deterministic",
                    "model": "deepseek-flash", "simulator_version": "v1.0.1-fix"},
...
def build_cb_seed(record: dict, seed_id: str, seed_rng: random.Random) -> dict:
    ...
    extraction_context = None
    if cut["end_index"] < 0:
        # v5.3：统一两轮前缀——无消息的退化对话也取前两条 seller 消息
        # （若存在）作 rollout 前缀，无空前缀特例
        extraction_context = []
        for i, e in enumerate(record["events"][:24]):
            if e["action"] == "message":
                role = ("seller" if kbs[int(e["agent"])]["personal"]["Role"]
                        == "seller" else "buyer")
                extraction_context.append({"role": role, "text_en": e["data"],
                                           "original_index": i})
        n_user = 0
        for m in extraction_context:
            prefix.append(m)
            if m["role"] == "seller":
                n_user += 1
                if n_user >= 2:
    ...
    for i, e in enumerate(record["events"]):
        if e["action"] == "message":
            role = ("seller" if kbs[int(e["agent"])]["personal"]["Role"]
                    == "seller" else "buyer")
            full_conversation.append({"role": role, "text_en": e["data"],
                                      "original_index": i})
    seed = {
        "seed_id": seed_id,
        ...
        "context": {"prefix": prefix, "full_conversation": full_conversation,
                    "extraction_context": extraction_context,
                    "task_events_in_prefix": task_events,
