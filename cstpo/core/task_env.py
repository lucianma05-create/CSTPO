...
# 注：v5.3 起种子的观测状态锚定在前两条 user 消息、潜在状态从全量前缀
# 推断，可见历史与认知初始化口径一致。
PREFIX_ROUNDS = 2
...
def merged_bdi(seed: dict) -> dict:
    """单一状态字段：initial_bdi 即模拟器初始可见输入的完整状态
    （观测 + 潜在已按 source_kind 标记合并，不再有独立 latent 字段）。"""
    bdi = seed.get("initial_bdi", {}) or {}
    return {k: list(bdi.get(k, [])) for k in ("beliefs", "desires", "intentions")}
...
class TaskEnv:
    ...
    def step(self, checkpoint: dict, utterance: str) -> dict:
        ...
        终止原因、成本。"""
        from cstpo.core.checkpoint import restore
