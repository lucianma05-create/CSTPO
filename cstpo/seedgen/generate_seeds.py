...
用法：
    cd CSTPO && python -m cstpo.seedgen.generate_seeds --task esconv --n 30
    （三任务全跑：--task all）
...
from cstpo.core.seed_adapter import (CHECKLIST, DEFAULT_PROFILE, EMOTION_MAP_V1,
                                sample_seeds)
...
1. The FULL conversation is provided as context (all turns, including those after
   the rollout start). Use all of it as evidence for the user's DISPOSITIONAL
   state — the standing subjective attitudes the user held at the start, which
   may only be revealed in later turns. The extraction target is the user's
   inherent state at the conversation start: attitudes genuinely held from the
   start are extractable regardless of when the evidence appears; attitudes
   that were CAUSED by the counterparty's actions are REACTIVE and excluded
   (see rule 4).
...
4. TWO-KIND STATE EXTRACTION (REV v6, 2026-09-20, 人工预审第 2 轮续)：
   完整 prefix 的全部轮次都可以作为证据。认知状态分两类，**都要提取**：
   (a) OBSERVED 已暴露：前两条 user 消息中已显性流露的状态——evidence 挂
       前两轮的索引，source_kind=observed；
   (b) LATENT 潜在主观：截止时用户真实持有的固有主观状态，前两轮未显露
       但后续轮次可推断（后轮反复流露的长期焦虑、稳定价值观、固有立场、
       长期愿望）——evidence 挂对应后轮索引，source_kind=latent。
   必须排除 REACTIVE 反应性状态——它们是状态转移的产物，模拟器会在交互
   中自然演化，不属于初始状态：
   (a) 对对方特定行为的即时评价反应（"Your offer of $X is too low /
       reasonable"），除非该评价表达的是用户固有的价格底线立场
       （如 "I would never sell below $300" 且前文显示这是长期立场）；
   (b) 被干预后的状态变化（"I feel much better after talking to you"——
       被安慰/说服后的新状态，不是截止时的初始状态）；
   (c) 未来条件性承诺（"I intend to offer $100 off if the buyer picks it
       up"），除非前两条 user 消息已显露该意图。
   判定口诀：条目描述"用户本来就是这样的人/本来就这样想"→ 提取（observed
   或 latent）；描述"因为对方做了什么，用户才变成这样"→ 排除。
...
def extract(llm: LLMClient, task: str, seed: dict) -> dict:
    # v5.4：标注上下文 = 全量对话（含截止点之后）；extraction_context /
    # prefix 仅为回退。截止点只决定 rollout 起点，不限制标注可见范围。
    ctx_msgs = (seed["context"].get("full_conversation")
                or seed["context"].get("extraction_context")
                or seed["context"]["prefix"])
    prefix_lines = []
    for p in ctx_msgs:
...
def critic_route(seed: dict, llm: LLMClient) -> None:
    ...
    from cstpo.seedgen.ablate_critic import CRITIC_SYSTEM, CRITIC_USER
...
def fill_seed(llm: LLMClient, task: str, seed: dict) -> dict:
    ...
    # v5.2（2026-09-20 人工预审第 2 轮终稿）：单一状态字段——观测 + 潜在
    # 全部留在 initial_bdi，每条按证据索引打 source_kind 标记
    # （前两条 user 消息内 = observed，更晚 = latent）。不再设
    # initial_bdi_latent 独立字段：全部条目都是模拟器初始可见输入，
    # 区分只是来源标记。
    try:
        prefix = seed.get("context", {}).get("prefix") or []
        user_idx = [i for i, m in enumerate(prefix)
                    if m.get("role") in ("user", "seller", "seeker", "persuadee")]
        vis = user_idx[1] if len(user_idx) >= 2 else (len(prefix) - 1)
        import re as _re
        for kind in ("beliefs", "desires", "intentions"):
            for it in bdi.get(kind, []):
                ev = str(it.get("evidence") or "")
                m = _re.search(r"prefix msg (\d+)", ev)
                if m and int(m.group(1)) > vis:
                    it["source_kind"] = "latent"
                else:
                    it.setdefault("source_kind", "observed")
        # v6.1 反应性机械过滤（人工预审 cb_14/17 裁定）：对特定报价的评价
        # （"The buyer's offer of $X is too low/reasonable"）是反应性状态，
        # 无论报价是否在前缀内都排除；固有底线立场（"I cannot go below
        # $X"、"my price is firm"）不匹配此模式，保留。提示词挡不住
        # LLM，机械层兜底。
        _offer_eval = re.compile(
            r"offer of \$?[\d,.]+ is (?:too (?:low|high)|(?:very )?reasonable"
            r"|unacceptable|a lowball)",
            re.IGNORECASE)
        for kind in ("beliefs", "desires", "intentions"):
            kept = []
            for it in bdi.get(kind, []):
                if _offer_eval.search(str(it.get("content_en", ""))):
                    seed["review"].setdefault("custom", {})
                    seed["review"]["custom"].setdefault("extraction_flags", [])
                    seed["review"]["custom"]["extraction_flags"].append(
                        f"reactive_drop_{kind}: {it.get('content_en','')[:60]}")
                    continue
                kept.append(it)
            bdi[kind] = kept
    except Exception:
    ...
    payload = {
        ...
        "prefix": [{"index": p["original_index"], "role": p["role"],
                    "text": p["text_en"]}
                   for p in (seed["context"].get("full_conversation")
                             or seed["context"]["prefix"])],
        "extraction_context": [{"index": p["original_index"], "role": p["role"],
                                "text": p["text_en"]}
                               for p in (seed["context"].get("extraction_context")
                                         or [])],
    }
