...
DEACTIVATE_THRESHOLD = 0.5   # 非核心节点强度低于该值则 deactivate（扩展规则，README 偏差第 6 条）
...
class BDIItem:
    ...
    content: str         # 用户第一人称表达（文档 §3.2）
    strength: float      # [0,4]
    core: bool = True    # 非核心节点优先淘汰（文档 §31 数量限制）
    active: bool = True
    # Desire/Intention 的极性：approach=希望实现/去做，avoid=希望避免/不去做。
    # Belief 不使用该字段（命题真伪无所谓极性）。
    # 使文档 §32.1 的 gc_i（促进/阻碍）可程序化计算，避免依赖文本解析。
...
class CognitiveProfile:
    """Theta_u = (eta_R, tau_A, tau_R)，全部保存在程序侧（文档 §6）。"""
...
@dataclass
class TurnLog:
    """单轮可审计日志（文档 §42、§45：状态可解释、过程可审计）。"""
    turn: int
    assistant_reply: str
    mode: str
    mode_reason: str | None
    route: str | None
    route_mode: str | None       # bernoulli / deterministic（文档 §17、§17.1）
    judgment: str | None
    p_central: float | None
    relevance: float | None      # 当轮三个 Route 特征（文档 §13，映射后数值）
    argument_strength: float | None
    cue_strength: float | None
    interaction_pressure: float | None   # §34 交互压力（low/medium/high -> 0.2/0.5/0.8）
    dominant_cues: list
    discrepancy: float | None
    target: str | None
    relevant_state_ids: list     # Discrepancy 输出的相关 B/D/I 节点 id（文档 §19）
    bdi_before: dict
    bdi_after: dict
    appraisal: dict
    emotion_before: dict
    ...
    desire_assessment: list = field(default_factory=list)   # LLM 逐 Desire 标注（§32.1 审计）
